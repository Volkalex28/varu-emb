...
impl<const COND: bool, const MSG: &'static str> IsTrue for AssertMsg<COND, MSG> where Msg<{ assert(COND, MSG) }>: IsTrue {}
impl<const COND: bool, const MSG: &'static str> IsFalse for AssertMsg<COND, MSG> where Msg<{ assert(!COND, MSG) }>: IsTrue {}
